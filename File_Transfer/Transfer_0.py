...
class ParentWindow(Frame):
    def __init__(self, master):
        ...
        self.btn_open = tk.Button(self.master,width=12,height=2,text='File Check',command=lambda: checkFiles(self))
        self.btn_open.grid(row=6,column=0,padx=(20,0),pady=(10,0),sticky=W)
        
        #text box to display file paths 
...
# script to check files for last modified time
# if files are modified within last 24 hours
# they are sent to folder2

def checkFiles(self):
    SECONDS_IN_DAY = 24 * 60 * 60
    source = self.txt_1.get()
    destination = self.txt_2.get()
    now = time.time()
